...
for i in range(0,4):
    if(i==3):
        dizhilist2_3[0][i] = 'http://www.hbjc.gov.cn/gzbg/ztbg/' + str(dizhilist2_3[0][i])
    else:
        dizhilist2_3[0][i] = 'http://www.hbjc.gov.cn/' + str(dizhilist2_3[0][i])
    ...
    for deepurl in url:
        ...
        if deepimg == []:
            ...
            for img in deepimg:
                ...
                img = 'http://www.hbjc.gov.cn/jcyw/{0}/{1}'.format(s, img)
                try:
                    urllib.request.urlretrieve(img, r'D:\text\工作报告\专题报告\{0}{1}.jpg'.format(deeptitle, k))
                except:
                    pass
        # 输出文本
        with open(r'D:\text\工作报告\专题报告\{0}.txt'.format(deeptitle), 'w', encoding="utf-8") as file:
            for texts in deeptext:
                if (texts[0] == '\r'):
                    if (texts[-1] == '\n'):
                        file.write(texts)
